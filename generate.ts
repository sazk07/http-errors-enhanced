...
import { toLowerFirst, toUpperFirst } from "./src/utils.js";
...
function buildError(
  ...
  return `export const ${toLowerFirst(identifier)}Schema = ${JSON.stringify(definition)}`;
}
...
async function main(): Promise<void> {
  ...
  for (const [code, message] of Object.entries(STATUS_CODES as Dictionary)) {
    ...
    const phrase = toUpperFirst((message as string).toLowerCase()) + ".";
    ...
    const errorConstant = identifier
